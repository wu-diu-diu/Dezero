...
class Layer:
    ...
    def __setattr__(self, name, value):  # __setattr__ 是一个特殊方法，在设置对象属性时会被调用。即父类和子类中设置self.xxx时调用
        if isinstance(value, (Parameter, Layer)):
            self._params.add(name)
        super().__setattr__(name, value)

    def __call__(self, *inputs):  # 使得实例可以像函数一样被调用
    ...
    def params(self):
        for name in self._params:
            obj = self.__dict__[name]  # 一个字典，存储了对象的所有属性,这里只存储了属性为Parameter类的属性，即W，b

            if isinstance(obj, Layer):
                yield from obj.params()  # 递归调用Layer
            else:
                yield obj  # return和yield的区别是，yield返回时for循环不会结束

    def cleargrads(self):
        for param in self.params():  # 此时param为variable变量，故可调用cleargrad()方法
            param.cleargrad()
...
class TwoLayerNet(Layer):  # 子类会继承父类所有的属性和方法，除非子类覆盖了这些方法
...
class RNN(Layer):
    ...
    def forward(self, x):
        if self.h is None:
            h_new = F.tanh(self.x2h(x))  # 第一次self.h是None，故输出仅由输入决定
        else:
            h_new = F.tanh(self.x2h(x) + self.h2h(self.h))  # 之后输出不仅由输入决定，同时也由上一次的输出决定
        self.h = h_new
        return h_new
