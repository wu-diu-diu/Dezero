...
def plot_dot_graph(output, verbose=True, to_file='graph.png'):
    ...
    with open(graph_path, 'w') as f:  ## 打开graph_path处文件，没有的话会创建
        f.write(dot_graph)

    extension = os.path.splitext(to_file)[1][1:]  # Extension(e.g. png, pdf) 返回to_file的扩展名
    cmd = 'dot {} -T {} -o {}'.format(graph_path, extension, to_file)  # cmd命令手动运行可以成功，可能是python环境的问题
    subprocess.run(cmd, shell=True)

    try:
        from IPython import display
        return display.Image(filename=to_file)  ## 这行用于在jupyter中展示
    except:
        pass
...
cache_dir = os.path.join(os.path.expanduser('~'), '.dezero')  # expanduser('~')会返回当前用户主目录即：C:\user\username
...
def get_file(url, file_name=None):
    ...
    """
    if file_name is None:  ## 如果没有提供文件名，则从最后一个斜杠后面的字符提取文件名
        file_name = url[url.rfind('/') + 1:]
    file_path = os.path.join(cache_dir, file_name)  ## 缓存路径和文件名构成下载地址

    if not os.path.exists(cache_dir):  ## cache_dir 缓存目录 /.dezero
        os.mkdir(cache_dir)

    if os.path.exists(file_path):  ## 如果地址已经存在，即文件已被下载，则函数返回文件地址
        return file_path
