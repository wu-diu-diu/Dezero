...
def pooling_simple(x, kernel_size, stride=1, pad=0):
    ...
    y = col.max(axis=1)  ## 此时y为大小为（N * OH * OW * C）
    y = y.reshape(N, OH, OW, C).transpose(0, 3, 1, 2)  ## 直接reshape可能会导致数据顺序错误
    return y
...
class Conv2d(Function):
    ...
    def forward(self, x, W, b):
        ...
        KH, KW = W.shape[2:]  # w的shape为 OC, C ,KH, KW
        col = im2col_array(x, (KH, KW), self.stride, self.pad, to_matrix=False)

        y = xp.tensordot(col, W, ((1, 2, 3), (1, 2, 3)))  ## 在1， 2， 3轴上进行点乘
        if b is not None:
            y += b
        y = xp.rollaxis(y, 3, 1)
